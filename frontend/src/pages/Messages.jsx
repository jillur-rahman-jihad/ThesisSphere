    ...
    <div className="flex h-full gap-6">
          ...
          {search.trim() !== '' && (
            <div className="mt-2 max-h-40 overflow-y-auto border rounded bg-white dark:bg-slate-800">
              {allUsers.filter(u => {
                ...
                <div key={u._id} className="p-2 hover:bg-slate-50 dark:bg-slate-900/50 cursor-pointer flex justify-between items-center" onClick={() => openConversation(u)}>
                  <div>
                    <div className="font-medium text-black">{u.fullName || u.name || u.email}</div>
                    <div className="text-xs text-slate-600 dark:text-slate-300">{u.email}</div>
                  </div>
                  <div className="text-sm text-slate-500 dark:text-slate-400">{u.role}</div>
                </div>
              ))}
              {allUsers.filter(u => {
                const q = search.toLowerCase();
                ...
                <div className="p-2 text-sm text-slate-600 dark:text-slate-300">No users found</div>
              )}
            </div>
          )}
        </div>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="text-sm text-slate-700 dark:text-slate-200 truncate" style={{ maxWidth: '220px' }}>{item.lastMessage}</div>
                  </div>
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-lg shadow p-4 flex flex-col text-slate-900 dark:text-white" style={{ maxHeight: '78vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-2" style={{ minHeight: 0 }}>
          {!selected ? (
            <div className="h-full flex items-center justify-center text-slate-600 dark:text-slate-300">Choose a conversation to start</div>
          ) : (
